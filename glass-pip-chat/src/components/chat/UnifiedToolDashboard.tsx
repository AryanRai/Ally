...
export function UnifiedToolDashboard({
    ...
    const [testMessages, setTestMessages] = useState<Array<{ id: string, role: 'user' | 'assistant', content: string, timestamp: number }>>([]);
    ...
    const renderTestingInterface = () => (
        <div className="space-y-6">
            ...
            <div className={cn(
                ...
                {/* Messages Area */}
                <div className={cn(
                    "h-96 overflow-y-auto p-4 space-y-3",
                    ThemeUtils.getBackgroundClass(platform, theme, 'secondary')
                )}>
                    {testMessages.length === 0 ? (
                        <div className="flex items-center justify-center h-full">
                            <div className="text-center">
                                <div className={cn("text-lg mb-2", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                                    🔧
                                </div>
                                <p className={cn("text-sm", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                                    Start testing your tools by typing a message below
                                </p>
                                <p className={cn("text-xs mt-1", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                                    Try: "calculate 2+2", "what time is it?", or "weather in London"
                                </p>
                            </div>
                        </div>
                    ) : (
                ...
                <div className={cn(
                    ...
                    >
                        <input
                            ...
                            placeholder="Type a message to test tool calling..."
                            ...
                            className={cn(
                                ...
                                ThemeUtils.getBorderClass(platform, theme),
                                ThemeUtils.getTextClass(platform, theme),
                                "placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
                            )}
                        />
                        <button
                            type="submit"
                            disabled={isTestProcessing || !testInput.trim()}
                            className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        >
                            {isTestProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Send'}
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div className={cn(
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className={cn(
                        ...
                        <p className={cn("text-xs", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                            Try: "calculate 2+2" or "what's 15 * 7?"
                        </p>
                    </div>
                    ...
                    <div className={cn(
                        ...
                        <p className={cn("text-xs", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                            Try: "what time is it?" or "current time"
                        </p>
                    </div>
                    ...
                    <div className={cn(
                        ...
                        <p className={cn("text-xs", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                            Try: "weather in London" or "how's the weather?"
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className={cn(
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <span className={cn("ml-1", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                                {testMessages.filter(m => m.role === 'user').length}
                            </span>
                        </div>
                        <div>
                            <span className={cn("font-medium", ThemeUtils.getTextClass(platform, theme))}>
                            ...
                            <span className={cn("ml-1", ThemeUtils.getTextClass(platform, theme, 'secondary'))}>
                                {testMessages.filter(m => m.role === 'assistant').length}
                            </span>
                        </div>
                        <div>
                            <span className={cn("font-medium", ThemeUtils.getTextClass(platform, theme))}>
                            ...
                            <span className={cn("ml-1 text-green-400")}>
                                Active
                            </span>
                        </div>
                    </div>
    ...
    const handleTestSubmit = async () => {
        ...
        try {
            ...
            if (testInput.toLowerCase().includes('calculate') || testInput.match(/\d+[\+\-\*\/]\d+/)) {
                response += "I've used the calculator tool to process your mathematical expression.";
            } else if (testInput.toLowerCase().includes('time')) {
                response += "I've used the current_time tool to get the current date and time.";
            } else if (testInput.toLowerCase().includes('weather')) {
                response += "I've used the weather tool to get weather information for your location.";
            } else {
                response += "I've processed your request using the available tools.";
            }
